...
// timeout are important for programs that connect to external resources
// or that otherwise need bound execution time. Implementing timeouts in go
// is easy and elegan thanks to channels and select
...
func main() {
    ...
    // so the send in the goroutine is nonblocking. This is a common pattern to
    ...
    // the timeout case if the operation takes more than the allowed 1.
	select {
	case res := <-c1:
		fmt.Println(res)
